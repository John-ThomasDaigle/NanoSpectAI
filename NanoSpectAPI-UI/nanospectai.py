...
# Minimum size to accept an image as a real inspection photo 
...
# PDF template with header/footer and table of contents 
class InspectionDocTemplate(BaseDocTemplate):
    ...
    def afterFlowable(self, flowable):
        ...
        Each finding has a zero-height invisible paragraph
...
def create_cover(
    ...
    if logo_path and os.path.exists(logo_path):
        logo_w, logo_h = 1.3 * inch, 0.8 * inch
    ...
    c.drawCentredString(width / 2, y_text, f"NANOSPECT AI — {building_type.upper()} INSPECTION REPORT")
    y_text -= 18
 ...
 # Divider line
    ...
    if inspectors:
       ...
       # Footer
        ...
        c.setLineWidth(0.5)
        c.line(margin, 0.52 * inch, width - margin, 0.52 * inch)
        c.drawCentredString(width / 2, 0.38 * inch, "This report was generated by NanoSpect AI. Please review the report for potential errors. ")
    c.showPage()
    c.save()
    logger.info("Cover page saved to: %s", output_path)
    
#Flatten the document body in reading order
...
def extract_image_caption_pairs(doc_path: str, image_temp_dir: str) -> list:
    ...
    relationship ID, and filters out small images 
    """
...
# Prevents API overload and hardcrashing if uploaded DOCX has excessive images
    MAX_FINDINGS_PER_CALL = 100
    if len(pairs) > MAX_FINDINGS_PER_CALL:
        raise RuntimeError(
            f"Too many findings ({len(pairs)}) for a single API call. "
            f"Please split the document into smaller sections of {MAX_FINDINGS_PER_CALL} or fewer images."
        )
    
    # Simple content check to catch if Gemini is referencing forbidden phrases
    FORBIDDEN_PHRASES = ["the image", "the photo", "the notes", "the caption"]
    ...
    item["severity"] = raw_sev if raw_sev in VALID_SEVERITIES else "Minor"
    ...
    try:
        ...
        for i, item in enumerate(pairs, start=1):
            ...
            # Title guardrail: must exist and be 2-5 words
            if not title or len(title.split()) > 8:
                title = "Inspection Finding"

            # Description guardrail: must be a real paragraph
